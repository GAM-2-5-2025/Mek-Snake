...
# Set the snake and food size to 20x20
snake_size = 20
...
def random_food_position():
    # Ensure food spawns on grid aligned with snake size
    min_x = margin
    max_x = window_x - margin - snake_size
    x = random.randrange(min_x, max_x + 1, snake_size)
   
    min_y = margin
    max_y = window_y - margin - snake_size
...
def main_game():
    ...
    while running:
        ...
        # Draw food with the same size as the snake
        pygame.draw.rect(game_window, white, pygame.Rect(food_position[0], food_position[1], snake_size, snake_size))
